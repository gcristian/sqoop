...
public class TestAppendUtils extends ImportJobTestCase {
   ...
   */
   protected ArrayList getOutputlessArgv(boolean includeHadoopFlags, String [] colNames,
      Configuration conf) {
    if (null == colNames) {
  ...
  private FileStatus[] listFiles(FileSystem fs, Path path) throws IOException {
	  ...
	  HashSet<FileStatus> hs=new HashSet();
  ...
  private String getFileCreationTimeImage(FileSystem fs, Path outputPath, int fileCount) throws IOException {	 
    ...
    for (int i=0; i<fileStatuses.length && i<fileCount;i++) {
   		image.append(fileStatuses[i].getModificationTime());
    }
  ...
  /**
   * Test for ouput path file-count increase, 
   * current files untouched and new correct partition number
   * @throws IOException
   */
  public void runAppendTest(ArrayList args, Path outputPath) throws IOException {
	...
	try {
	    ...
	    assertTrue("Output directory didn't got increased in file count ", currentFileCount>previousFileCount);
	    
	    // check previous files weren't modified, also works for partition overlapping 
	    ...
	    Path newFile=fileStatuses[previousFileCount].getPath(); // there is a new bound now
  ...
  public void testAppend() throws IOException {
	ArrayList args=getOutputlessArgv(false, HsqldbTestServer.getFieldNames(), getConf());
